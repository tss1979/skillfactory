...
class Board:
    ...
    def add_ship(self, ship: Ship):
        ...
        is_fill = any([dot in self.all_ships_dots for dot in ship.dots])
        is_out = any([self._out(dot) for dot in ship.dots])
        if not is_out and len(self.ships) <= 7 and not is_neib_fill and not is_fill:
        ...
        elif any([dot not in self.all_ships_dots for dot in ship.dots]):
    ...
    def create_user_board(self) -> None:
        ...
        for j, size in enumerate(SIZES):
            while len(self.user_board.ships) != j + 1:
                ...
                try:
                    x = int(input('Введите коррдинату начальной точк по горизонтали от 1 до 6: '))
                    y = int(input('Введите коррдинату начальной точк по вертикале от 1 до 6: '))
                    if size == 1:
                        dir = "влево"
                    else:
